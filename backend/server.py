...
# Vercel's cleanUrls strips the .html extension from static links (e.g. a
# request for /html/explore.html gets 308-redirected to /html/explore). The
# StaticFiles mount below only matches literal filenames with the extension,
# so that redirected request would 404. This route is registered BEFORE the
# mount so it gets first chance to match, restores the extension, and serves
# the page for both /html/explore and /html/explore.html.
